...
BBR was developed by Cardwell et al. \cite{BBRORIGINAL} to address fundamental inefficiencies in traditional loss-based \acs{CCA}s. 
Cardwell et al. argue that modern hardware advancements have made the long-standing practice of using packet loss as a measure for congestion increasingly problematic. 
...
Cardwell et al. define the optimal operating point as having a delivery rate equal to the bottleneck bandwidth ($BtlBw$) and a total amount of inflight data equal to the pipe's \ac{BDP} \cite{BBRORIGINAL}. 
...
Li et al. \cite{AQDCBBR} identify a critical limitation: these transient queues may not be completely drained during the subsequent ProbeDown phase, leading to a residual queue buildup that accumulates over successive cycles. 
...
Furthermore, Li et al. contend that BBR's ProbeRTT mechanism is rigid and ``blind'' to queue history, as it resets at fixed intervals regardless of the actual backlog. 
...
While \ac{BBR} ProbeRTT phase is intended to synchronize flows and provide a "fair starting point" by draining the queue, Han et al. found that it often fails because the algorithm lacks an explicit mechanism to correct accumulated queue imbalances upon exiting this state. 
Consequently, prior imbalances are immediately reintroduced and amplified, a phenomenon Han et al. describe as the "inaction" of BBR.

To resolve these issues, Han et al. introduced BBR-ES, which modifies the state machine by inserting a short stabilization phase, BBR-NEW-STATE, between ProbeRTT and ProbeBW. 
...
Dong et al. solved this by re-architecting congestion control to rely on empirical evidence gathered through continuous "micro-experiments" rather than predetermined responses. 
...
This approach allows the protocol to adapt to complex network conditions without making prior assumptions, and Dong et al. further improved decision-making accuracy under noisy conditions by utilizing randomized controlled trials (RCTs) to confirm the causal connection between a rate change and its observed utility.

\subsection{Copa}

Similar to the motivation behind BBR, Arun et al. observed that traditional loss-based \acs{CCA}s often cause significant "bufferbloat" at the network bottleneck\cite{COPA}. 
This excessive queuing delay severely degrades the performance of interactive applications that require low latency. 
Arun et al. argued that existing delay-based algorithms, such as TCP Vegas or FAST TCP, are insufficient for modern internet conditions.
...
Furthermore, while newer "objective-oriented" or machine-generated algorithms like PCC may offer high performance, Arun et al. noted they are often "black boxes" that are difficult for humans to reason about or tune.

To address these limitations, Arun et al. introduces Copa which is built upon three core design pillars. 
...
Jiang et al. \cite{COPAPLUS} identified several critical vulnerabilities in the original Copa algorithm through mathematical analysis. 
...
To address these issues, Jiang et al. \cite{COPAPLUS} introduced Copa+, which incorporates a parameter adaptation mechanism and a refined mode-switching logic. 
...
Furthermore, Jiang et al. optimized the criteria for entering Competitive Mode by utilizing the algorithm's inherent oscillation cycles. 
...
Haile et al. \cite{COPADYNAMIC} focus their research on the unique challenges of highly volatile mobile networks, such as 4G and 5G and named their \ac{CCA} Copa-D. 
...
While acknowledging the approach taken by Copa+ (which introduces an additional parameter to the rate control equation) Haile et al. propose with Copa-D an alternative strategy. 
Rather than altering the underlying control structure, they focus on the core sensitivity parameter, $\delta$.
In the original Copa implementation, $\delta$ is typically treated as a static constant, which Haile et al. argue is the primary cause of its inability to adapt to cellular fluctuations. 
...
Addanki et al. \cite{PowerTCP} developed PowerTCP which focuses primarily on the unique demands of datacenter networks. 
...
Furthermore, Addanki et al. \cite{PowerTCP} anticipate a future where datacenter switches feature increasingly shallow buffers, significantly smaller than those found in traditional internet infrastructure. 
...
Given these evolving circumstances, Addanki et al. conclude that a hybrid congestion control approach—integrating both "voltage" and "current" signals—is necessary to maintain stability while achieving near-instantaneous reaction times.
...
Addanki et al. implement this via a window update law.
...
Huitema et al. \cite{C4BlogPost} outlined three primary objectives for their algorithm which they called C4. 
...
Huitema et al. specifically cite reports of degraded video quality in high-density Wi-Fi environments, such as buildings with many concurrent users. 
This focus on unstable connectivity significantly influenced key design decisions. 
Finally, Huitema et al. emphasized architectural simplicity and intended for the algorithm to be deployed primarily in conjunction with the QUIC protocol.
